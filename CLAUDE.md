...
> **Version**: 2.6.39 · `versionCode` 470 · **Updated**: September 12, 2026 (G4 Release C prep: versioned net-target + executable exit-policy contract. Training/live orders still frozen; p_ml entry gate unchanged. Live SHADOW_* remains POSITION_POLICY_V1 gross.)
...
- **Version markers:** `ENTRY_ELIGIBILITY_VERSION = entry_eligibility_v5_quote_friction_fail_closed`; `NET_ECONOMICS_VERSION = net_economics_v2_executable_quote_contract`; `TEACHER_FRICTION_VERSION = teacher_friction_v2_executable_bid_ask_charges`; `BRAIN_VERSION = "2.6.39"`; `POSITION_EXIT_POLICY_CONTRACT_VERSION = position_exit_policy_v1_net_20260912`; `NET_RANK_EDGE_SCALE = net_premium_edge_absolute_rupees_after_executable_friction`.
