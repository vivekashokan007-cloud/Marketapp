...
> **Version**: 2.6.25 · `versionCode` 456 · **Updated**: September 10, 2026 (repairs atomic multi-batch evaluator append so its copied JSON prefix is not truncated. No ranking, Paper/Real, model, or broker behavior changes.)
...
- **Version markers:** `ENTRY_ELIGIBILITY_VERSION = entry_eligibility_v5_quote_friction_fail_closed`; `NET_ECONOMICS_VERSION = net_economics_v2_executable_quote_contract`; `TEACHER_FRICTION_VERSION = teacher_friction_v2_executable_bid_ask_charges`; `BRAIN_VERSION = "2.6.25"`; `NET_RANK_EDGE_SCALE = net_premium_edge_absolute_rupees_after_executable_friction`.
