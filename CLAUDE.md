...
> **Version**: 2.6.5 · `versionCode` 436 · **Updated**: August 30, 2026 (selector observability closed at code level + predicted-vs-realized net-objective artifact; brain/Kotlin versions synchronized; remote PWA remains separate)
...
### Candidate selection — PC2 paper primary selector (v6, brain.py)
...
2. `rank_edge_effective` — **absolute net premium edge, after the far-OTM sigma de-rate** (see below). This is the primary economic authority as of v6 (`PC2_PAPER_PRIMARY_SELECTOR_VERSION = "pc2_paper_primary_v6"`), replacing `adjustedEdgePerRisk`/composite score as tie-breaker-only signals. This was an explicit empirical decision, not a stylistic one: on a realistic top-2-picks/day basis (21 trading days, leave-one-day-out), net-edge selection beat edge-per-risk selection (which measured worst, -0.17R to -0.28R) and beat a look-ahead-contaminated "adaptive percentile" variant that only looked positive because its reference window could see future data — rebuilt causally (prior sessions only), it dropped to -0.0504. Telemetry note: `rankEdgeScale` now explicitly says this is absolute rupees after executable friction, not a per-risk ratio; gross fallback is labeled separately.
...
> **Selector telemetry note.** Before v2.6.5, `primary_candidate_json` dropped the flat PC2/net fields (`netPremiumEdge`, `netEconomicsVersion`, `sigmaPenaltyFactor`, `rankEdgeEffective`, `sigmaOTM`) even when they were computed correctly. v2.6.5 closes this at code level by persisting flat mirrors (`netPremiumEdge`, `rankEdgeEffective`, `rankEconomicsBasis`, `sigmaPenaltyFactor`, `sigmaExcessOverCeiling`, `frictionCost`, `netEconomicsVersion`, `entryEligible`, `entryConfidence`) plus nested `pc2PaperSortComponents` and `entryEligibility`. Production row proof is still required after the phone reports v2.6.5/b436; until then, use `top_candidates_json -> pc2PaperSortComponents` as fallback evidence for older rows.

Items fixed by the v2.6.0 audit (M2.2, M3.3, M1.1, the sigma de-rate, the v6 selector) have been removed from this list — see "Audit fixes shipped in v2.6.0" above. The v2.6.3 executable quote/friction fix is also closed at code level; v2.6.4 corrected the rank-edge telemetry label and added resumable research tooling; v2.6.5 closes P1 selector observability at code level. Remaining live gaps:
...
6. **P1 selector observability is closed at code level in v2.6.5/b436; production proof pending.** Confirm fresh `ml_brain_snapshots.primary_candidate_json` rows persist `netPremiumEdge`, `rankEdgeEffective`, `sigmaPenaltyFactor`, `rankEconomicsBasis`, `entryEligibility.reasons`, and `pc2PaperSortComponents` after the phone updates. Older rows still require `top_candidates_json -> pc2PaperSortComponents` fallback evidence.
...
8. **Production verification after v2.6.5 is pending.** Confirm the phone reports v2.6.5/b436, the native app/brain versions match, `rankEdgeScale` uses absolute-rupee wording, generated candidates with incomplete executable bid/ask fail closed with `quote_incomplete` / `NET_UNAVAILABLE_FAIL_CLOSED`, and fresh primary rows contain the v2.6.5 selector telemetry mirrors. Remote PWA versioning is separate because this APK wraps `MarketVivi` instead of bundling web assets.
...
GitHub Actions/CI status for this repo could not be verified from the auditing session (separate sandbox API restriction, distinct from the git-push path) — confirm the signed-release workflow succeeded and that the phone reports v2.6.5/b436 before trusting new production data rows as v2.6.5 evidence.
