...
| R4 — silent one-lot default | **Fixed locally** | Kotlin now distinguishes an omitted lot count from a supplied count, carries `number_of_lots_assumed` and its versioned policy through position-tick metadata, and rejects present invalid values. Python already stamped the same policy. |
| R5 — nested JSON `code` authorizes table fallback | **Fixed locally** | `extractPostgrestCode` now parses a JSON object and accepts only a top-level code. A nested `details.code=PGRST205` regression case must not trigger fallback. |
